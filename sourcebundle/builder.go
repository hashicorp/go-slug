...
func (b *Builder) findComponentPackageSource(ctx context.Context, sourceAddr sourceaddrs.ComponentSource, allowedVersions versions.Set) (sourceaddrs.RemoteSource, error) {
	...
	// // TODO: Validate latest-version or pinned only
	// selectedVersion := allowedVersions.List().Newest()
	log.Print("[DEBUG] go-slug selected version for findComponentPackageSource= ", selectedVersion.String())
	...
	if !ok {
		log.Print("[DEBUG] go-slug about to call registryClient.ComponentPackageSourceAddr=", " pkgAddr=", pkgAddr.String(), " version=", selectedVersion.String())
